    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            ...
            if len(x) > 0:
                ...
                if self.verbose > 0:
                    print(
                        f"Num timesteps: {self.num_timesteps} - Curr mean success: {mean_success:.2f} - Curr mean spl: {mean_spl:.2f}"
                    )
                    print(
                        f"Best mean success: {self.best_mean_success:.2f} - Last mean success per episode: {mean_success:.2f}"
                    )
                    print(
                        f"Best mean spl: {self.best_mean_spl:.2f} - Last mean spl per episode: {mean_spl:.2f}"
                    )
